...
suite('RPC provider', function () {
    ...
    setup(function () {
        local = new RpcProvider(
            (message, transfer) => (
                (transferLocalToRemote = transfer), remote.dispatch(message)
            ),
            50
        );
        ...
        remote = new RpcProvider(
            (message, transfer) => (
                (transferRemoteToLocal = transfer), local.dispatch(message)
            ),
            50
        );
    ...
    suite('RPC', function () {
        ...
        test('RPC handlers can return promises', function () {
            remote.registerRpcHandler(
                'action',
                () => new Promise(r => setTimeout(() => r(10), 15))
            );
        ...
        test('Invalid RPC calls throw on both ends', function () {
            return local
                ...
                .then(() => (assert(errorLocal), assert(errorRemote)));
        });
        ...
        test('Multiple RPC handlers do not interfere', function () {
            remote.registerRpcHandler(
                'a1',
                (value: number) =>
                    new Promise(r => setTimeout(() => r(value), 30))
            );
            remote.registerRpcHandler('a2', (value: number) => 2 * value);

            return Promise.all([local.rpc('a1', 10), local.rpc('a2', 20)]).then(
                ([r1, r2]) => (
        ...
        test('RPC handler can be deregistered', function () {
            ...
            return local.rpc('action').then(
                () => Promise.reject('should have been rejected'),
                () => (assert(errorLocal), assert(errorRemote))
            );
        });
